 ...
 *
 * This keeps the OpenAI API key off the client. The frontend calls:
...
function pickOrigin(headers) {
  ...
  // Reflecting the origin is typically friendlier than "*" for some setups.
  return origin || "*";
}

function buildDevHavenInstructions() {
  // Keep this short and grounded in the site. The assistant should not invent facts.
  return [
...
exports.handler = async function handler(event) {
  ...
  if (!apiKey) {
    return json(
      ...
      {
        error:
          "OPENAI_API_KEY is not set on the server. Add it as an environment variable in your hosting dashboard."
      },
      corsHeaders
    );
  }
